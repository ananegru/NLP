...
def initialize_matrix_of_counts(file, word_index_dict):
...
def build_mle_bigram(file, word_index_dict):
    counts = initialize_matrix_of_counts(file, word_index_dict)
    probs = normalize(counts, norm='l1', axis=1)
    write_out_probabilities(probs)
    return probs

file = open("brown_100.txt")
probs = build_mle_bigram(file, word_index_dict)
...
calculate_sentence_probability(toy_corpus, output_file, probs)
...
def generate_sentences(amount, file_name, word_index_dict):
    with open(file_name, "w") as file:
        ...
        for i in range(amount):
            generated_sentence = GENERATE(word_index_dict, probs, "bigram", 10, "the")
            # Write the generated sentence to the file
            file.write("Generated Sentence " + str(i + 1) + ": " + generated_sentence + "\n")
generate_sentences(10, 'bigram_generation.txt', word_index_dict)
